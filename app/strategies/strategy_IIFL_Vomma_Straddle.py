...
logger = create_logger(__name__, 'trading_IIFL_Vomma_straddle_strategy.log')


class StrategyStraddleIIFLVomma(BaseStrategy):
    def __init__(self, params):
        ...
        self.temp_file  = "open_position_IIFL_Vomma.csv"
        self.PROFIT_CUT_QTY = 65
        self.ROUND_OFF      = 50
        ...
        self.RAS_1SEC_LOOKBACK  = 5 * 60   # bars for acceleration live (300 ticks)
        self.RAS_1SEC_NORM      = 20 * 60  # bars for normalisation live (1200 ticks)
        
        self.VOMMA_INTRADAY_THRESHOLD = 3.5
    ...
    def place_sell_order(self, CE_SELL_PRICE,PE_SELL_PRICE,call_instrument_id, put_instrument_id, quantity, exchange="NSEFO"):
        try:
            ...
            if Rc is None:
                logger.info(f"[Error] Placing Call Sell order Failed: {call_order_id}")
            
            if Rp is None:
                logger.info(f"[Error] Placing Put Sell order Failed: {put_order_id}")

            # logger.info(f"[SELL] CE: {Rc} | PE: {Rp} | Total: {Rc+Rp}")
            resp = float(Rp + Rc)
    ...
    def square_off_straddle(self, CE_BUY_PRICE,PE_BUY_PRICE,call_instrument_id, put_instrument_id, quantity, exchange="NSEFO"):
        try:
            ...
            if Rc is None:
                logger.info(f"[Error] Placing Call Buy order Failed: {call_order_id}")
            
            if Rp is None:
                logger.info(f"[Error] Placing Put Buy order Failed: {put_order_id}")

            # logger.info(f"[BUY/SquareOff] CE: {Rc} | PE: {Rp} | Total: {Rc+Rp}")
            resp = float(Rp + Rc)
    ...
    def load_entry_data_from_csv(self):
        try:
            ...
            with open(self.temp_file, "r") as f:
                ...
                row["entry_vomma"] = float(row.get("entry_vomma", 0.0)) 
    ...
    def write_to_csv(self, data):
        headers = [
            ...
            'Entry_RAS', 'Exit_RAS', 'RAS_KillSwitch','Entry_Vomma', 'Exit_Vomma'          
        ]
        try:
            try:
                            ...
                            if spot is not None:
                                ...
                                if self.live_ltp["CE"] and self.live_ltp["PE"]:
                                    ...
                                    if entry_premium > 50:
                                        ...
                                        CE_SP=self.live_ltp["CE"]-1.5
                                        PE_SP=self.live_ltp["PE"]-1.5
                                        ...
                                        entry_net_vomma = preentry_history[-1]["net_vomma"] if preentry_history else 0.0
                            ...
                            if self.live_ltp["CE"] and self.live_ltp["PE"]:
                                ...
                                if spot is not None:
                                    ...
                                    if tte_live > 0:
                                        ...
                                        live_net_vomma = live_snap["net_vomma"] 
                                        self.exit_vomma=live_net_vomma
                                        ...
                                        print(f"[Vomma Live] {live_net_vomma:.4f} ")
                                ...
                                if (pnl > 15 and self.entry_data["cut"] == 0):
                                    ...
                                    CE_BP=self.live_ltp["CE"]+1.5
                                    PE_BP=self.live_ltp["PE"]+1.5
                                ...
                                # ── Layer 1: Vomma Kill Switch ──────────────────
                                entry_vomma = self.entry_data.get("entry_vomma", 0.0)
                                if live_net_vomma > (entry_vomma * self.VOMMA_INTRADAY_THRESHOLD):
                                    ...
                                    Exit_Type   = "VOMMA_KILL"
                                    
                                    self.deactivate_ws(self.entry_data["atm"])
                                    Ti = now.strftime("%H:%M:%S")
                                    logger.info(
                                        f"[VOMMA KILL] Time:{Ti} | Vomma:{live_net_vomma:.4f} | "
                                        f"ExitPrem:{exit_prem:.2f} | PnL:{final_pnl:.2f} | "
                                        f"Total:₹{Total:.2f}"
                                    )
                                    print(f"[VOMMA KILL] Spiked from {entry_vomma:.4f} to {live_net_vomma:.4f} @ {Ti} | PnL=₹{Total:.2f}")

                                    # --- AUTOMATED DATABASE LOGGING FOR VOMMA KILL EXIT ---
                                    try:
                                        new_trade = TradeNotification(
                                            user_id=deployed_strategy.user_id,
                                            strategy_id=deployed_strategy.strategy_id,
                                        ...
                                        logger.error(f"Failed to log Vomma Kill Exit Trade Notification: {e}")
