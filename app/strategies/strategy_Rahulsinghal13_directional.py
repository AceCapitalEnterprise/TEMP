...
logger = create_logger(__name__, 'trading_Rahulsinghal13_directional_orb_strategy.log')


class StrategyRahulsinghal13DirectionalOrb(BaseStrategy):
    def __init__(self, params):
        ...
        self.PATH_CE = "unclosed_positions_Rahulsinghal13_directional_ce.csv"
        self.PATH_PE = "unclosed_positions_Rahulsinghal13_directional_pe.csv"
    ...
    def place_options_order_sliced(self,total_qty, slice_qty, action,
                                order_type, price=None,
                                right=None, strike_price=None, expiry_date=None):
        ...
        if strike_price is None:
            raise ValueError("strike_price is required for option orders.")
        if expiry_date is None:
            raise ValueError("expiry_date is required for option orders.")
        ...
        for i in range(num_slices):
            try:
                ...
                time.sleep(2)
                print(f"Slice {i+1} response: {response}")
            except Exception as e:
                logger.error(f"Error placing Slice {i+1}order for {right} {strike_price}: ", exc_info = e)
        ...
        if remainder > 0:
            try:
                ...
                time.sleep(2)
                print(f"Remainder slice response: {response}")
            except Exception as e:
                logger.error(f"Error placing order for {right} {strike_price}: ", exc_info = e)
                print(f"Error in remainder slice: {e}")
        if remainder == 0:
            return response   
   

    def square_off_order_sliced(self,total_qty, slice_qty, action,
                                order_type, price=None,
                                right=None, strike_price=None, expiry_date=None):
        ...
        if remainder > 0:
            try:
                ...
                logger.error(f"Error placing remainder slice order for {right} {strike_price}: ", exc_info = e)
                print(f"Error in remainder slice: {e}")
        if remainder == 0:
            return response
   
    # Passed db_session and deployed_strategy here
    def update_trailing_sl(self, positions_df, path, db_session, deployed_strategy):
        try:
            ...
            for index, position in positions_df.iterrows():
                ...
                if current_price >= position['trailing_sl']:
                    price_buy=self.leg_premium(position['CE_or_PE'],position['strike']) 
                    order_detail = self.square_off_order_sliced(
                                total_qty=self.QTY,
                                slice_qty=self.SLICE_QTY,
                                action="buy",
                                order_type="limit",
                                price=(price_buy+1),
                    ...
                    execution_price = float(sell_pe_price[0])
                    ...
                    # --- AUTOMATED DATABASE LOGGING FOR EXIT (Trailing SL Hit) ---
                    try:
                        new_trade = TradeNotification(
                            ...
                            action="BUY", 
                            symbol="NIFTY",
                            instrument_detail=f"{position['strike']} {str(position['CE_or_PE']).upper()}",
                        ...
                        logger.error(f"Failed to log Exit Trade Notification: {e}")
    ...
    # Passed db_session and deployed_strategy here
    def check_profit_target_and_add_position(self, positions_df, path, ce_or_pe, db_session, deployed_strategy):
        ...
        try:
            ...
            if open_position < self.MAX_POSITION and self.TIME_1 < now <= self.TIME_2 and current_price is not None and current_price <= target_price and self.ADDING_POS:
                ...
                if 10 < nearest_premium < 12.5:
                    order_detail = self.place_options_order_sliced(
                                total_qty=self.QTY,
                                slice_qty=self.SLICE_QTY,
                                action="sell",
                                order_type="limit",
                    ...
                    order_detail = pd.DataFrame(order_detail)
                    sell_pe_price = order_detail['execution_price']
                    leg_price = float(sell_pe_price[0])
                    print(f"Leg Price for {ce_or_pe}: {leg_price}")
                    ...
                    # --- AUTOMATED DATABASE LOGGING FOR ENTRY (Adding New Position) ---
                    try:
                        new_trade = TradeNotification(
                            user_id=deployed_strategy.user_id,
                            strategy_id=deployed_strategy.strategy_id,
                            ...
                            action="SELL",
                            symbol="NIFTY",
                            instrument_detail=f"{strike} {str(ce_or_pe).upper()}",
                            price=leg_price,
                            qty=int(self.QTY)
                        )
                        db_session.add(new_trade)
            ...
            try:
                ...
                while deployed_strategy.status == "RUNNING":
                    try:
                                        ...
                                        if closest_strike_pe:
                                            ...
                                            if last['close'] <= sup and 10 < last['close'] < 12.5:
                                                ...
                                                order_detail = self.place_options_order_sliced(
                                                            ...
                                                            price="9",
                                                            right="put",
                                                            strike_price=str(closest_strike_pe),
                                                            expiry_date=f'{self.EXPIRY}T06:00:00.000Z'
                                                )
                                                ...
                                                order_detail = pd.DataFrame(order_detail)
                                                sell_pe_price = order_detail['execution_price']
                                                entry_premium = float(sell_pe_price[0])
                                                pnl=entry_premium
                                                position = {
                                                ...
                                                # --- AUTOMATED DATABASE LOGGING FOR ENTRY (Initial Put) ---
                                                try:
                                                    new_trade = TradeNotification(
                                                        user_id=deployed_strategy.user_id,
                                                        strategy_id=deployed_strategy.strategy_id,
                                                        ...
                                                        action="SELL",
                                                        symbol="NIFTY",
                                                        instrument_detail=f"{closest_strike_pe} PUT",
                                                        price=entry_premium,
                                                        qty=int(self.QTY)
                                                    )
                                                    db_session.add(new_trade)
                                        ...
                                        if closest_strike_ce:
                                            ...
                                            if last['close'] <= sup and 10 < last['close'] < 12.5:
                                                ...
                                                order_detail = self.place_options_order_sliced(
                                                            ...
                                                            price="9",
                                                            right="call",
                                                            strike_price=str(closest_strike_ce),
                                                            expiry_date=f'{self.EXPIRY}T06:00:00.000Z'
                                                )
                                                ...
                                                # --- AUTOMATED DATABASE LOGGING FOR ENTRY (Initial Call) ---
                                                try:
                                                    new_trade = TradeNotification(
                                                        user_id=deployed_strategy.user_id,
                                                        strategy_id=deployed_strategy.strategy_id,
                                                        ...
                                                        action="SELL",
                                                        symbol="NIFTY",
                                                        instrument_detail=f"{closest_strike_ce} CALL",
                                                        price=entry_premium,
                                                        qty=int(self.QTY)
                                                    )
                                                    db_session.add(new_trade)
                        ...
                        if not self.positions_df_pe.empty:
                            print(now)
                            # Passing db_session and deployed_strategy here
                            self.positions_df_pe = self.update_trailing_sl(self.positions_df_pe, self.PATH_PE, db_session, deployed_strategy)
                            self.positions_df_pe = self.check_profit_target_and_add_position(self.positions_df_pe, self.PATH_PE, "put", db_session, deployed_strategy)
                        ...
                        if not self.positions_df_ce.empty:
                            print(now)
                            # Passing db_session and deployed_strategy here
                            self.positions_df_ce = self.update_trailing_sl(self.positions_df_ce, self.PATH_CE, db_session, deployed_strategy)
                            self.positions_df_ce = self.check_profit_target_and_add_position(self.positions_df_ce, self.PATH_CE, "call", db_session, deployed_strategy)
