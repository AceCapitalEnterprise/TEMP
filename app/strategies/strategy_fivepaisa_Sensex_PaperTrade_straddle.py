...
logger = create_logger(__name__, 'trading_straddle_strategy_sensex_PaperTrade.log')

class StrategyStraddleSensexPaperFivePaisa(BaseStrategy):
    def __init__(self, params):
        ...
        self.temp_file="open_position_Sensex_Straddle_paper.csv"
        self.PROFIT_CUT_QTY=160
        self.ROUND_OFF=100
...
# <--------------------- Placing order ------------------->
    ...
    def place_order_bulk(self,side):
        try:
            ...
            Rp=None
            Rc=None
            CE_order=self.client.place_order(OrderType=order_type,Exchange='B',ExchangeType='D', ScripCode = self.TOKENS["CE"], Qty=self.QTY, Price=0)
            print(f"CE order details : {CE_order}")
            PE_order=self.client.place_order(OrderType=order_type,Exchange='B',ExchangeType='D', ScripCode = self.TOKENS["PE"], Qty=self.QTY, Price=0)
    ...
    def square_off(self,Q):
        ...
        for pos in positions:
            if pos['NetQty'] != 0 and pos['ScripCode']==self.TOKENS["CE"]:
                CE_order=self.client.place_order(OrderType='B',Exchange='B',ExchangeType='D', ScripCode = self.TOKENS["CE"], Qty=Q, Price=0)
                print(f"CE order details : {CE_order}")

            if pos['NetQty'] != 0 and pos['ScripCode']== self.TOKENS["PE"]:   
                PE_order=self.client.place_order(OrderType='B',Exchange='B',ExchangeType='D', ScripCode = self.TOKENS["PE"], Qty=Q, Price=0)
                print(f"PE order details : {PE_order}")
        time.sleep(2)
        orders = self.client.order_book()
        for order in orders:
...
# <---------------------------------- For Checking if Entry is Already exit ------------------------------>
    ...
    def check_atr_adx(self):
        try:
            now = datetime.now()
            x=t(14, 10)
            hist = self.client.historical_data("B", "C", 999901, "1m", (now - timedelta(days=4)).date(), now.date())
    ...
    def check_GAP(self):
        try:
            now = datetime.now()
            hist = self.client.historical_data("B", "C", 999901, "1d", (now - timedelta(days=10)).date(), now.date())
            df = pd.DataFrame(hist)
            df["datetime"] = pd.to_datetime(df["Datetime"])
            df.set_index("datetime", inplace=True)
            if len(df) < 3:
    ...
    def check_nearest_ltp(self,atm,Cc,Cu,Cd,Pc,Pu,Pd):
        try:
            req_list = [
            {"Exch":"B","ExchType":"D","ScripCode":Cc},
            {"Exch":"B","ExchType":"D","ScripCode":Cu},
            {"Exch":"B","ExchType":"D","ScripCode":Cd},
            {"Exch":"B","ExchType":"D","ScripCode":Pc},
            {"Exch":"B","ExchType":"D","ScripCode":Pu},
            {"Exch":"B","ExchType":"D","ScripCode":Pd}
            ]
            ...
            print(f"Call: | Call Curr: {Cc}-{cc} | Call Up: {Cu} -{cu}| Call Down: {Cd}-{cd}")
            ...
            if m==m1 and m<=(0.2*max(cc,pc)):
                ...
                return Cd,Pd,atm-100
            elif m==m3 and m<=(0.2*max(cu,pu)):
                return Cu,Pu,atm+100
            else:
                print("Failed to do premium Matching")
                return None,None,None
            
        except Exception as e:
            print(f"[ERROR] Premium Fetching Failed : {e}")
            logger.error(f"[ERROR] Premium Fetching Failed : {e}")
            return None,None,None
    ...
    def start_feed(self):
        try:
            ...
            req = self.client.Request_Feed("mf", "s", [
                {"Exch": "B", "ExchType": "D", "ScripCode": self.TOKENS["CE"]},
                {"Exch": "B", "ExchType": "D", "ScripCode": self.TOKENS["PE"]}
            ])
...
# <---------------------------------- For Checking if Entry is Already exit ------------------------------> 
    ...
    def get_scrip_code(self, strike, opt_type):
        try:
            strike = str(strike)
            df = self.client.query_scrips("B", "D", "SENSEX", strike, opt_type, self.expiry1)
            if not df.empty:
                    ...
                    try:
                        ...
                        if self.TIME_1 < now.time() < self.TIME_2 and GAP and self.max_trades>0 and self.entry_data is None and now.second == 0:
                            ...
                            if spot is not None:
                                ...
                                if self.live_ltp["CE"] and self.live_ltp["PE"]:
                                    ...
                                    end_hist = now
                                    hist_ce = self.client.historical_data('B', 'D', self.TOKENS['CE'], '1m', start_hist.date(), end_hist.date())
                                    hist_pe = self.client.historical_data('B', 'D', self.TOKENS['PE'], '1m', start_hist.date(), end_hist.date())
                        ...
                        # TRAIL STOP LOSS CHECK
                            ...
                            if self.live_ltp["CE"] and self.live_ltp["PE"]:
                                ...
                                if (pnl>25 and self.entry_data["cut"]==0) or (pnl>50 and self.entry_data["cut"]==1):
