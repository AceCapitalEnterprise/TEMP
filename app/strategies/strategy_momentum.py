...
from ..models.db_models import TradeNotification  # Added Trade Import 
...
class StrategyMomentumLive(BaseStrategy):
    ...
    def fetch_month_data(self,symbol, today, interval='day'):
        try:
            ...
            data = self.kite.historical_data(instrument_token, "2024-01-01", today, interval)
    ...
    def create_or_update_portfolio(self, selected_date, daily_stocks_data, last_day_of_month, db_session, deployed_strategy):
        selected_date = pd.to_datetime(selected_date).date()
        ...
        for stock, weight, price in zip(stocks_to_hold, weights, current_prices):
            shares = int(round((weight * self.INITIAL_INVESTMENT) / price)) if price > 0 else 0
            if stock in current_stocks:
                ...
                # --- AUTOMATED DATABASE LOGGING FOR ENTRY ---
                try:
                    ...
                    logger.error(f"Failed to log Entry Trade for {stock}: {e}")
    ...
    def check_intra_month_exits(self, portfolio_df, first_day, last_day, daily_stocks_data, db_session, deployed_strategy):
            ...
            if not stock_data.empty:
                ...
                for _, data_row in stock_data.iterrows():
                     if data_row['SMA_22'] < data_row['SMA_66'] or (
                        ...
                        # --- AUTOMATED DATABASE LOGGING FOR EXIT (Intra-month) ---
                        try:
                            exit_price = float(data_row['close'])
                            shares = int(row['Number of Shares']) if not pd.isna(row['Number of Shares']) else 0
                            ...
                            logger.error(f"Failed to log Intra-Month Exit Trade for {stock}: {e}")
                        break
        if exits:
    ...
    def check_month_end_exits(self, portfolio_df, last_day, daily_stocks_data, db_session, deployed_strategy):
        ...
        for _, row in portfolio_df.iterrows():
            if row['Symbol'] not in top_stocks:
                ...
                # --- AUTOMATED DATABASE LOGGING FOR EXIT (Month-End) ---
                try:
                    stock_data = daily_stocks_data[
                        (daily_stocks_data['Symbol'] == row['Symbol']) & 
                        (daily_stocks_data['Date'] == last_day)
                    ]
                    exit_price = stock_data['close'].iloc[0] if not stock_data.empty else float(row['Current Price'])
                    shares = int(row['Number of Shares']) if not pd.isna(row['Number of Shares']) else 0
                    ...
                    logger.error(f"Failed to log Month-End Exit Trade for {row['Symbol']}: {e}")
    ...
    def run(self, deployed_strategy_id: int) -> None:
        ...
        log_msg = f"========== STARTING EXECUTION #{self.run_count} AT {current_time} =========="
        print(log_msg)
        logger.info(log_msg)
        ...
        with Session(engine) as db_session:
            ...
            # 1. Guard Check: Ensure the strategy hasn't been stopped by the user
            if not deployed_strategy or deployed_strategy.status != "RUNNING":
                print(f"Strategy {deployed_strategy_id} is stopped. Aborting scheduled run.")
            ...
            try:
                ...
                if isinstance(self.first_day_of_month, str):
                ...
                if isinstance(self.last_day_of_month, str):
                ...
                if all_data:
                    ...
                    print("No data fetched. Exiting scheduled run.")
                    logger.info(f"[INFO]: No data fetched. Exiting scheduled run.")
                    return
                        
                # 3. Evaluate Portfolio Logic (Passing db_session and deployed_strategy)
                if today == self.first_day_of_month:
                    ...
                    portfolio_df = self.create_or_update_portfolio(self.first_day_of_month, daily_stocks_data, self.last_day_of_month, db_session, deployed_strategy)
                    ...
                    if os.path.exists(self.csv_file):
                        portfolio_df = pd.read_csv(self.csv_file)
                        portfolio_df = self.check_month_end_exits(portfolio_df, self.last_day_of_month, daily_stocks_data, db_session, deployed_strategy)
                    else:
                    ...
                    if os.path.exists(self.csv_file):
                        portfolio_df = pd.read_csv(self.csv_file)
                        portfolio_df = self.check_intra_month_exits(portfolio_df, self.first_day_of_month, self.last_day_of_month, daily_stocks_data, db_session, deployed_strategy)
                    else:
                ...
                print("YAY! We have successfully reached the end of this interval!")    
                logger.info(f"[INFO]: YAY! We have successfully reached the end of this interval!") 
                
            except Exception as e:
                logger.error(f"Error during scheduled execution:", exc_info=e)
                print(f"Scheduled execution error: {str(e)}")