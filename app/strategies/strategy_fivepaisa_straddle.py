...
logger = create_logger(__name__, 'trading_straddle_strategy.log')

class StrategyStraddleFivePaisa(BaseStrategy):
    def __init__(self, params):
        ...
        self.temp_file="open_position.csv"
        self.PROFIT_CUT_QTY=260
        self.ROUND_OFF=50
...
# <---------------------------------- Data For Greeks calculation ------------------------------>
    ...
    def calculate_greeks_sl(self,ent_time,exi_time, nifty, strike, time_to_expiry, volatility, atr,option_type):
        ...
        logger.info(f"Delta : {delta}  ||  Theta :  {theta}  ||  Vega:  {vega}")
...
# <---------------------------------- For Checking if Entry is Already exit ------------------------------>
    ...
    def check_atr_adx(self):
        try:
            ...
            # prev1 = df.iloc[-2]
            # prev2 = df.iloc[-3]
            # atr_down = prev2["ATR"] > prev1["ATR"] > last["ATR"]
            # adx_down = prev2["ADX"] > prev1["ADX"] > last["ADX"]
            # rv_down=prev2["RV"] > prev1["RV"] > last["RV"]
            # print(f"[DEBUG] ATR: {prev2['ATR']:.2f} > {prev1['ATR']:.2f} > {last['ATR']:.2f} => {atr_down}")
            # print(f"[DEBUG] ADX: {prev2['ADX']:.2f} > {prev1['ADX']:.2f} > {last['ADX']:.2f} => {adx_down}")
            # print(f"[DEBUG] RV: {prev2['RV']:.2f} > {prev1['RV']:.2f} > {last['RV']:.2f} => {rv_down}")
            if len(df) < 15:
                return None
            last = df.iloc[-5]
            prev10 = df.iloc[-15]
    ...
    def check_nearest_ltp(self,atm,Cc,Cu,Cd,Pc,Pu,Pd):
        try:
            ...
            logger.info(f"Call: | Call Curr: {Cc}-{cc} | Call Up: {Cu} -{cu}| Call Down: {Cd}-{cd}")
            logger.info(f"Put:  | Put Curr: {Pc}-{pc} | Put Up: {Pu}-{pu} | Put Down: {Pd}-{pd}")
            m1=abs(cc-pc)
            m2=abs(cd-pd)
            m3=abs(cu-pu)
            m=min(m1,m2,m3)
            print(f" C:{m1} | D:{m2} | U:{m3} | M:{m}")
            logger.info(f" C:{m1} | D:{m2} | U:{m3} | M:{m}")
