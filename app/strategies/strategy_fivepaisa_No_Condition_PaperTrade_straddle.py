...
logger = create_logger(__name__, 'trading_straddle_strategy_No_Condition_PaperTrade.log')

class StrategyStraddleNoConditionPaperFivePaisa(BaseStrategy):
    def __init__(self, params):
        ...
        try:
            ...
            logger.info(f"Session Token:{params['session_token']}")
        ...
        self.temp_file="open_position_no_condition_paper.csv"
        self.ROUND_OFF=50
                ...
                while deployed_strategy.status == "RUNNING":
                    try:
                        ...
                        if self.TIME_1 < now.time() < self.TIME_2 and GAP and self.max_trades>0 and self.entry_data is None and now.second == 0:
                            ...
                            spot,R = self.check_atr_adx()
                            if spot is not None and R is not None and (R<=0.11):
                                atm = self.round_to_nearest_50(spot)
                                print(f"spot: {spot} || atm : {atm}")
                                print("Fetching tokens...")

                                self.TOKENS["CE"] = self.get_scrip_code(atm, "CE")
                                self.TOKENS["PE"] = self.get_scrip_code(atm, "PE")
                                ...
                                if self.live_ltp["CE"] and self.live_ltp["PE"]:
                                    ...
                                    tte=(expiry_dt - now)/timedelta(days=1)/365
                                    print("<---++++++-->")
