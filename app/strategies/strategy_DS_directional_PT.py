...
logger = create_logger(__name__, 'trading_PT_directional_orb_strategy.log')


class StrategyDirectionalPTOrb(BaseStrategy):
    def __init__(self, params):
        ...
        # Global Variables
        self.tick_data = {}
        self.positions_df_ce = pd.DataFrame(columns=['datetime', 'action', 'strike', 'CE_or_PE', 'premium', 'trailing_sl','ltp'])   # Change For powerbi update 
        self.positions_df_pe = pd.DataFrame(columns=['datetime', 'action', 'strike', 'CE_or_PE', 'premium', 'trailing_sl','ltp'])   # Change For powerbi updtae 
        self.ws_connected = False
        self.last_ltp_update_time = time.time()
    ...
    def place_options_order_sliced(self,total_qty, slice_qty, action,
        ...
        for i in range(num_slices):
            try:
                ...
                # print(data)
                if data['Success']:
                    execution_price = float(data['Success'][0]['ltp'])
            except Exception as e:
            ...
            if open_position < self.MAX_POSITION and self.TIME_1 < now <= self.TIME_2 and current_price is not None and current_price <= target_price and self.ADDING_POS:
                ...
                if 10 < nearest_premium < 12.5:
                    ...
                    new_position = {
                        ...
                        'ltp': leg_price,   # Change For powerbi update 
                    }
            ...
            try:
                ...
                while deployed_strategy.status == "RUNNING":
                    try:
                                        ...
                                        if closest_strike_pe:
                                            ...
                                            if last['close'] <= sup and 10 < last['close'] < 12.5:
                                                ...
                                                position = {
                                                    ...
                                                    'ltp': entry_premium,   # Change For powerbi update 
                                                }
                        ...
                        #  Change For powerbi update  
                        current_time = time.time()
                        print(f"current_time: {current_time} | last_ltp_update_time : {self.last_ltp_update_time}")
                        if current_time - self.last_ltp_update_time >= 15:
                            print("HI")
                            # with self.lock:
                            if not self.positions_df_pe.empty:
                                for idx, row in self.positions_df_pe.iterrows():
                                    curr_price = self.get_current_market_price('put', row['strike'])
                                    print(f"Current Price: {curr_price} ")
                                    if curr_price is not None:
                                        self.positions_df_pe.at[idx, 'ltp'] = curr_price
                                self.positions_df_pe.to_csv(self.PATH_PE, header=True, index=False)
                                
                            if not self.positions_df_ce.empty:
                                for idx, row in self.positions_df_ce.iterrows():
                                    curr_price = self.get_current_market_price('call', row['strike'])
                                    print(f"Current Price: {curr_price} ")
                                    if curr_price is not None:
                                        self.positions_df_ce.at[idx, 'ltp'] = curr_price
                                self.positions_df_ce.to_csv(self.PATH_CE, header=True, index=False)

                            self.last_ltp_update_time = current_time
