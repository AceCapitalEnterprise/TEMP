...
@router.get("/reports/{strategy_id}/view-detail", response_class=JSONResponse)
async def view_strategy_detail(
    strategy_id: int,
    current_user: Annotated[User, Depends(auth_service.get_current_user)],
    db_session: Session = Depends(db.get_session)
):
...
@router.get("/reports/{strategy_id}/view-report", response_class=JSONResponse)
async def view_strategy_report(
    strategy_id: int,
    current_user: Annotated[User, Depends(auth_service.get_current_user)],
    db_session: Session = Depends(db.get_session)
):
    strategy = strategy_service.get_strategy_by_id(db_session, strategy_id)
    # Target the report file specifically
    file_name = strategy.file_name.replace(".csv", "_report.csv")
    file_path = settings.csv_folder_path + file_name
    
    if not os.path.exists(file_path):
        return JSONResponse({"error": "Report CSV file not found."}, status_code=404)
    ...
    try:
        with open(file_path, mode='r', encoding='utf-8') as f:
            ...
            data = list(reader)
            if not data:
                return JSONResponse({"error": "Report CSV file is empty."}, status_code=400)
