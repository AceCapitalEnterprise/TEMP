...
async def strategies(
    ...
    all_strategies = strategy_service.get_all_strategies(db_session)

    # Filter out strategies that do not match the selected category
    results = [s for s in all_strategies if getattr(s, "category", "Buying") == category]
    ...
    menu_title = f"{category} Strategies"
...
async def deploy_strategy(
    ...
    try:
        ...
        # --- NEW: Look up the category to redirect correctly ---
        db_strategy = strategy_service.get_strategy_by_id(db_session, strategy_id)
        category = getattr(db_strategy, "category", "Buying") if db_strategy else "Buying"  
        return RedirectResponse(url=f"/strategies?category={category}", status_code=303)
...
async def add_strategy(
    ...
    try:
        ...
        # --- NEW CODE: Auto-assign the new strategy to user "Sanjay" ---
        
        sanjay_user = db_session.exec(select(User).where(User.name == "Sanjay")).first()
        if sanjay_user:
            existing_sanjay = user_service.get_user_strategies_by_user_id_and_strategy_id(
                db_session, sanjay_user.id, new_strategy.id
            )
            if not existing_sanjay:
                user_service.create_user_strategy(db_session, sanjay_user.id, new_strategy.id)
        # ---------------------------------------------------------------
